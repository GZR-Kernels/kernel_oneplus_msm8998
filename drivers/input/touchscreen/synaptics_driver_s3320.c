...
#define KEY_GESTURE_DOUBLE_SWIPE    251 // swipe two finger vertically
#define KEY_GESTURE_DOWN_VEE        255 // ^
#define KEY_GESTURE_RIGHT_VEE       253 // <
#define KEY_GESTURE_LEFT_VEE        254 // >
#define KEY_GESTURE_UP_VEE          252 // v
#define KEY_GESTURE_RIGHT_SWIPE	    KEY_F5 // -->
#define KEY_GESTURE_LEFT_SWIPE	    KEY_F6 // <--
#define KEY_GESTURE_DOWN_SWIPE	    KEY_F7 // |v
#define KEY_GESTURE_UP_SWIPE	    KEY_F8 // |^
...
static void synaptics_get_coordinate_point(struct synaptics_ts_data *ts)
{
	...
	uint16_t trspoint = 0;
/* add by lifeng 2016/1/19 workarounds for the gesture two interrupts begin*/
	static uint8_t coordinate_buf_last[25] = {0};
/* add by lifeng 2016/1/19 workarounds for the gesture two interrupts end*/
...
/* add by lifeng 2016/1/19 workarounds for the gesture two interrupts begin*/
	...
	sizeof(coordinate_buf))) {
		TPD_ERR("%s reject the same gesture[%d]\n", __func__, gesture);
		gesture = UnkownGesture;
	}
...
static void gesture_judge(struct synaptics_ts_data *ts)
{
	...
	case SWIPE_DETECT:
		if (version_is_s3508) {
			...
			(regswipe == 0x44) ? Up2DownSwip :
			(regswipe == 0x48) ? Down2UpSwip :
			(regswipe == 0x84) ? DouSwip : UnkownGesture;
		}
		break;
	case CIRCLE_DETECT:
		gesture = Circle;
	...
	case UNICODE_DETECT:
		...
		(gesture_buffer[2] == 0x73) ? Sgesture : UnkownGesture;
	}
	...
	case DouTap:
		...
		break;
	case UpVee:
		keyCode = KEY_GESTURE_UP_VEE;
		break;
	case DownVee:
		keyCode = KEY_GESTURE_DOWN_VEE;
		break;
	case LeftVee:
		keyCode = KEY_GESTURE_LEFT_VEE;
		break;
	case RightVee:
		keyCode = KEY_GESTURE_RIGHT_VEE;
		break;
	case Circle:
	...
	case DouSwip:
		...
		break;
	case Left2RightSwip:
		keyCode = KEY_GESTURE_RIGHT_SWIPE;
		break;
	case Right2LeftSwip:
		keyCode = KEY_GESTURE_LEFT_SWIPE;
		break;
	case Up2DownSwip:
		keyCode = KEY_GESTURE_DOWN_SWIPE;
		break;
	case Down2UpSwip:
		keyCode = KEY_GESTURE_UP_SWIPE;
		break;
	case Mgesture:
	...
	TPD_ERR("detected %s gesture, sending keycode %d\n",
	...
	if (gesture != UnkownGesture && ts->gesture_enable != 0) {
...
#ifdef SUPPORT_TP_TOUCHKEY
#define OEM_KEY_LEFT (key_switch ? KEY_APPSELECT : KEY_BACK)
#define OEM_KEY_RIGHT (key_switch ? KEY_BACK : KEY_APPSELECT)
#else
#define OEM_KEY_LEFT KEY_APPSELECT
#define OEM_KEY_RIGHT KEY_BACK
#endif
...
static int	synaptics_input_init(struct synaptics_ts_data *ts)
{
	...
	set_bit(KEY_GESTURE_LEFT_SWIPE, ts->input_dev->keybit);
	set_bit(KEY_GESTURE_RIGHT_SWIPE, ts->input_dev->keybit);
	set_bit(KEY_GESTURE_W, ts->input_dev->keybit);
	set_bit(KEY_GESTURE_M, ts->input_dev->keybit);
	set_bit(KEY_GESTURE_UP_SWIPE, ts->input_dev->keybit);
	set_bit(KEY_GESTURE_DOWN_SWIPE, ts->input_dev->keybit);
